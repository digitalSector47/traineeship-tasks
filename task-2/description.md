...
4. Приложение должно собираться с помощью `Webpack`, `Vite` или `Gulp`.
5. Версия `Node.js LTS`.
6. Исходный код приложения должен быть читаемым.
7. Верстка должна отображаться корректно в трех последних версиях браузеров и устройствах.
8. В «.gitignore» добавлена папка «node_modules».
9. Писать коммиты по правилам [«Соглашения о коммитах»](https://www.conventionalcommits.org/en/v1.0.0/).
